...
const keralaFoods = [
  ...
  { name: "idli", alias: null, kcal_per: 70, protein_g: 2, carbs_g: 14, fat_g: 0.5, fiber_g: 0.5, serving: "1 piece (50g)" },
  ...
  { name: "beef fry", alias: "beef ularthiyathu", kcal_per: 280, protein_g: 25, carbs_g: 5, fat_g: 18, fiber_g: 1, serving: "1 serving (100g)" },
  { name: "chicken curry", alias: null, kcal_per: 200, protein_g: 22, carbs_g: 6, fat_g: 10, fiber_g: 1, serving: "1 serving (150g)" },
  { name: "egg roast", alias: "mutta roast", kcal_per: 160, protein_g: 12, carbs_g: 5, fat_g: 11, fiber_g: 1, serving: "2 eggs with masala" },
  { name: "sambar", alias: null, kcal_per: 90, protein_g: 4, carbs_g: 14, fat_g: 2, fiber_g: 3, serving: "1 cup (200ml)" },
  { name: "avial", alias: null, kcal_per: 130, protein_g: 3, carbs_g: 15, fat_g: 7, fiber_g: 4, serving: "1 cup (150g)" },
  { name: "thoran", alias: "stir fry vegetable", kcal_per: 80, protein_g: 2, carbs_g: 10, fat_g: 4, fiber_g: 3, serving: "1 serving (100g)" },
  { name: "olan", alias: null, kcal_per: 100, protein_g: 2, carbs_g: 12, fat_g: 5, fiber_g: 3, serving: "1 cup (200g)" },
  ...
  { name: "unniyappam", alias: null, kcal_per: 90, protein_g: 1.5, carbs_g: 16, fat_g: 2.5, fiber_g: 0.5, serving: "1 piece (35g)" },
  { name: "banana", alias: "Kerala banana", kcal_per: 110, protein_g: 1.5, carbs_g: 28, fat_g: 0.5, fiber_g: 3, serving: "1 medium (120g)" },
  { name: "coconut chutney", alias: null, kcal_per: 80, protein_g: 1, carbs_g: 4, fat_g: 7, fiber_g: 2, serving: "2 tbsp (30g)" },
  { name: "sulaimani tea", alias: "black lemon tea", kcal_per: 15, protein_g: 0, carbs_g: 4, fat_g: 0, fiber_g: 0, serving: "1 cup (200ml)" },
  { name: "coconut milk", alias: null, kcal_per: 120, protein_g: 1, carbs_g: 3, fat_g: 12, fiber_g: 0, serving: "100ml" },
...
async function main() {
  console.log("Seeding Kerala food database...");
  for (const food of keralaFoods) {
    await prisma.keralaFood.upsert({
      where: { name: food.name },
      update: food,
      create: food,
    });
  }
  console.log(`Seeded ${keralaFoods.length} Kerala foods.`);

  console.log("Seeding user profile (Ahmed)...");
  const hashedPassword = await bcrypt.hash("fittrack123", 10);
  await prisma.user.upsert({
    where: { email: "ahmed@fittrack.local" },
    update: {},
    create: {
      name: "Ahmed",
      email: "ahmed@fittrack.local",
  ...
  console.log("Seeded user: ahmed@fittrack.local / fittrack123");
}
