...
import { getTodayWorkouts, logWorkout, deleteWorkout } from "../api/client";
...
export default function Workouts() {
  ...
  const fetchWorkouts = async () => setWorkouts(await getTodayWorkouts());
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-100 p-6 mb-6 shadow-sm">
        ...
        <button
          type="submit"
          disabled={saving || !form.exercise || !form.sets || !form.reps}
              ...
              <div key={w.id} className="bg-white border border-slate-200 rounded-xl px-5 py-4 flex items-center justify-between group shadow-sm">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {w.sets} sets × {w.reps} reps{w.weight_kg ? ` @ ${w.weight_kg}kg` : ""}
                    {w.notes ? ` · ${w.notes}` : ""}
                  </p>
                </div>
                <button
                  onClick={() => handleDelete(w.id)}
                  className="text-slate-300 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 text-xl cursor-pointer"
                >
                  ×
                </button>
              </div>
